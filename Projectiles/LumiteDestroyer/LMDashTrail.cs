...
namespace SunksBossChallenges.Projectiles.LumiteDestroyer
{
    public class LMDashTrail:ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            projectile.Loomup();
            if (Util.CheckNPCAlive<LumiteDestroyerHead>((int)projectile.ai[1]) && Main.npc[(int)projectile.ai[1]].ai[1] == LumiteDestroyerSegment.PlanetAurora
                && Util.CheckProjAlive<LMDoublePlanetAurora>((int)Main.npc[(int)projectile.ai[1]].ai[3]))
            {
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Color lightColor)
        {
                ...
                && Util.CheckProjAlive<LMDoublePlanetAurora>((int)Main.npc[(int)projectile.ai[1]].ai[3]))
            {
                NPC head = Main.npc[(int)projectile.ai[1]];
                Projectile aurora = Main.projectile[(int)head.ai[3]];
                if ((head.Center - aurora.Center).Compare(LumiteDestroyerArguments.R) <= 0)
                {
